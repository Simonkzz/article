...
####################################################################################################
####################################################################################################
FILEPATHLF = './FinalData/sub/RFColumns.xlsx'
dataLR = pd.read_excel(FILEPATHLF)
StrideLRList = pd.DataFrame(dataLR)
StrideLRList[StrideLRList.RFP > 0.8].to_excel("./FinalData/sub/sub/RRRRFFFColumns.xlsx", index=False)
data1 = pd.read_excel("./FinalData/sub/sub/RRRRFFFColumns.xlsx")
...
plt.title("Right forefoot, relationship between Distance, Speed and Frequency")
