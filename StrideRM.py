...
####################################################################################################
####################################################################################################
FILEPATHLF = './FinalData/sub/RMColumns.xlsx'
dataLR = pd.read_excel(FILEPATHLF)
StrideLRList = pd.DataFrame(dataLR)
StrideLRList[StrideLRList.RMP > 0.8].to_excel("./FinalData/sub/sub/RRRRMMMColumns.xlsx", index=False)
data1 = pd.read_excel("./FinalData/sub/sub/RRRRMMMColumns.xlsx")
...
plt.title("Right middle foot, relationship between Distance, Speed and Frequency")
