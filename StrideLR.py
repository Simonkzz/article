...
####################################################################################################
####################################################################################################
FILEPATHLF = './FinalData/sub/LRColumns.xlsx'
dataLR = pd.read_excel(FILEPATHLF)
StrideLRList = pd.DataFrame(dataLR)
StrideLRList[StrideLRList.LRP > 0.8].to_excel("./FinalData/sub/sub/LLLLRRRColumns.xlsx", index=False)
data1 = pd.read_excel("./FinalData/sub/sub/LLLLRRRColumns.xlsx")
...
plt.title("Left hind foot, relationship between Distance, Speed and Frequency")
...
# 步频的长度是,记录的是帧数，根据帧数，每100帧一秒，计算频率 62
# 速度的区间是 0.5273495152747597 42.138686686585764
# 步幅的区间是
# 62 61 62