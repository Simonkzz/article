...
####################################################################################################
####################################################################################################
FILEPATHLF = './FinalData/sub/LMColumns.xlsx'
dataLM = pd.read_excel(FILEPATHLF)
StrideLMList = pd.DataFrame(dataLM)
StrideLMList[StrideLMList.LMP > 0.8].to_excel("./FinalData/sub/sub/LLLLMMMColumns.xlsx", index=False)
data1 = pd.read_excel("./FinalData/sub/sub/LLLLMMMColumns.xlsx")
...
plt.title("Left middle foot, relationship between Distance, Speed and Frequency")
...
# plt.plot(x1, np.linspace(np.mean(y), np.mean(y), 80), c='blue')
...
# 步频的长度是,记录的是帧数，根据帧数，每100帧一秒，计算频率 80
# 速度的区间是 0.5457909531714131 113.49098688611585
# 步幅的区间是 0.20372458210172528 4.819253644449145
# 80 79 80